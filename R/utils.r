...
sdi <- function(observations, B = 1000) {
  p <- as.numeric(table(observations)) / length(observations)
  estimate <- -sum(p*log(p))
  if (B == 0) {
    return(estimate)
  } else {
    bs_estimate <- sapply(1:B, function(i) sdi(sample(x = observations, replace = TRUE), 0))
    bs_q <- quantile(bs_estimate, probs = c(0.025, 0.975))
    return(data.frame(sdi = estimate,
                      sdi_low = bs_q[1],
                      sdi_high = bs_q[2]))
  }
}
# downsampled SDI
sdi_ds <- function(observations, N = 100, B = 1000) {
  return(sapply(1:B, function(i) sdi(sample(x = observations, size = N, replace = FALSE), 0)))
}
...
composition_plots <- function(df, group_var, comp_var, group_name, comp_name,
  ...
  return(list(data = df_sum, figure = p))
}

# test whether the co-occurence of x_val and y_val is greater than 
# expected by chance in the paired observations x_vec and y_vec
enrich_test <- function(x_val, y_val, x_vec, y_vec) {
  x <- factor(x_vec == x_val, levels = c(FALSE, TRUE))
  y <- factor(y_vec == y_val, levels = c(FALSE, TRUE))
  tab <- table(x, y)
  test_res <- fisher.test(tab)
  res <- data.frame(odds_ratio = test_res$estimate, 
...
run_hyper <- function(genes, background, pathways, min_size = 9, 
                      max_size = 500, collapse = TRUE) {
  pathways <- lapply(pathways, function(x) intersect(x, background))
  pathways_size <- sapply(pathways, length)
  pathways <- pathways[pathways_size >= min_size & pathways_size <= max_size]
  message('run_hyper: ', length(genes), ' of ', length(background), ' genes, ',
          length(pathways), ' pathways')
  if (length(genes) < 1 || length(pathways) < 1) {
  ...
  hyp_res <- hypeR::hypeR(signature = genes, 
                          genesets = pathways, 
                          test = 'hypergeometric', 
                          background = background, 
                          quiet = TRUE)
  hyp_res <- hyp_res$as.data.frame() %>% arrange(pval) 
  if (collapse & nrow(hyp_res) > 1) {
    n <- nrow(hyp_res)
    
    # check how much of a pathway is contained in a higher ranking one
    tmp <- matrix(0, n, n)
    for (i in 2:n) {
      pw_i <- hyp_res$label[i]
      size_i <- length(pathways[[pw_i]])
      for (j in 1:(i-1)) {
        pw_j <- hyp_res$label[j]
        n_com <- length(intersect(pathways[[pw_i]], pathways[[pw_j]]))
  ...
  mutate(hyp_res, label = gsub(pattern = ' \\(GO:\\d+\\)$', replacement = '', x = label))
}
...
edgeR_conserved_plot_volcano <- function(de, facet_var = 'split_level', ncol = 3) {
  p1 <- ggplot(de$res, aes(logFC, -log10(pval_clipped))) + 
    geom_point(aes(color = FDR < 0.05)) + 
    geom_point(data = de$top_markers, color = "deeppink") + 
    geom_text_repel(data = de$top_markers, mapping = aes(label = gene)) + 
    facet_wrap(~ get(facet_var), ncol = ncol, scales = 'free_y') +
