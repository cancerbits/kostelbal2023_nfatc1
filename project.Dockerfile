...
RUN R -e "BiocManager::install('treemap')"
...
RUN conda create -c conda-forge -n snakemake bioconda::snakemake bioconda::snakemake-minimal --only-deps \
  && conda clean --all -y
ENV PATH /opt/mambaforge/envs/snakemake/bin:${PATH}
